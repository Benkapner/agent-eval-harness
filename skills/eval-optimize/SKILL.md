...
| `--model <model>` | no | `models.skill` from eval.yaml | Model for eval runs |
...
| `--split <train:sel:test>` | no | `20:10:70` | Dataset split ratio |
...
Build a failure map: `judge_name → [case_ids] → rationale for each`
...
```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/agent_eval/state.py append tmp/rejected-edits.yaml \
  "iteration=<N> edit_summary='<what>' category='<type>' score_before=<X> score_after=<Y> reason='selection gate'"
```
...
total = sum(json.load(open(f)).get('cost_usd', 0) or 0
            for f in glob.glob('$AGENT_EVAL_RUNS_DIR/<id>-iter-*/run_result.json'))
...
- **Every edit must be grounded in evidence** — cite judge, cases, transcript evidence, support count
...
- **Stop after max iterations** — report what couldn't be fixed
- **Don't modify test cases, judges, or eval.yaml** — improve the skill, not the evaluation
- **Try different approaches** — if the same edit category fails twice, try a different category
